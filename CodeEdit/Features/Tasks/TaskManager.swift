...
final class TaskManager {
	...
	var activeTasks: [UUID: CEActiveTask] = [:]
	private var userSelectedTaskID: UUID?
	var taskShowingOutput: UUID?
...
extension TaskManager {
	var automaticallySelectedTaskID: UUID? {
		guard selectedTask == nil else { return nil }
		return availableTasks.first?.id
	}

	var selectedTaskID: UUID? {
		get { userSelectedTaskID ?? automaticallySelectedTaskID }
		set { userSelectedTaskID = newValue }
	}
	...
	var currentTaskStatus: CETaskStatus? {
		guard let selectedTaskID else { return nil }
		return activeTasks[selectedTaskID]?.status
	}
